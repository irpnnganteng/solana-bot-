...
/setmetadata — buat metadata token (nama, simbol, logo)
/lock — kunci otoritas mint selamanya (fixed supply, PERMANEN)
/cancel — batalkan konfirmasi yang sedang menunggu

Aksi berbahaya (mint besar, transfer, lock, setmetadata) akan meminta konfirmasi ketik *ya* sebelum dieksekusi.`;
...
bot.onText(/\/setmetadata/, (msg) => {
  ...
  bot.sendMessage(
    msg.chat.id,
    `Kamu akan membuat metadata token:\nNama: LEON\nSimbol: LEO\n\n` +
      `Ketik *ya* untuk konfirmasi (berlaku 60 detik), atau /cancel untuk batal.`,
    { parse_mode: 'Markdown' }
  );
});
...
bot.on('message', async (msg) => {
  ...
  try {
    ...
    if (pending.action === 'setmetadata') {
        ...
        },
        {
          createMetadataAccountArgsV3: {
            data: {
              name: 'LEON',
              symbol: 'LEO',
              uri: 'https://azure-gentle-wren-659.mypinata.cloud/ipfs/bafkreidh2wvvldpwangdt2b5e5jdg3gva3m2iv6nyeohnmx4m5oqog76g4',
      ...
      bot.sendMessage(msg.chat.id, `✅ Metadata berhasil dibuat!\nTx: https://solscan.io/tx/${sig}`);
    }
